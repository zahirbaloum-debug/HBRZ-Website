          ...
          <div className="hidden lg:flex items-center lg:space-x-4 xl:space-x-6 ml-4">
            <Link
              ...
              className={`text-[11px] xl:text-xs font-bold uppercase tracking-widest transition-colors duration-200 whitespace-nowrap ${
                isActive('/') ? 'text-hbrz-gold border-b-2 border-hbrz-gold' : 'text-gray-500 hover:text-hbrz-blue'
              } py-2`}
            >
              Home
            </Link>
            ...
            >
              <button
                className={`flex items-center text-[11px] xl:text-xs font-bold uppercase tracking-widest transition-colors duration-200 whitespace-nowrap ${
                  location.pathname === '/about' || (location.pathname.startsWith('/about') && location.pathname !== '/about/careers') ? 'text-hbrz-gold border-b-2 border-hbrz-gold' : 'text-gray-500 hover:text-hbrz-blue'
                } py-2 focus:outline-none`}
              >
                About Us
                <svg className={`ml-1 h-3 w-3 transition-transform duration-200 ${aboutDropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div 
                className={`absolute top-full left-0 w-64 bg-white border border-gray-100 shadow-xl py-4 transition-all duration-200 origin-top transform ${
                  aboutDropdownOpen ? 'scale-y-100 opacity-100 visible' : 'scale-y-95 opacity-0 invisible'
                }`}
              >
            ...
            >
              <button
                className={`flex items-center text-[11px] xl:text-xs font-bold uppercase tracking-widest transition-colors duration-200 whitespace-nowrap ${
                  location.pathname.startsWith('/services') ? 'text-hbrz-gold border-b-2 border-hbrz-gold' : 'text-gray-500 hover:text-hbrz-blue'
                } py-2 focus:outline-none`}
              >
                Services
                <svg className={`ml-1 h-3 w-3 transition-transform duration-200 ${servicesDropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              ...
              <div 
                className={`absolute top-full left-0 w-80 bg-white border border-gray-100 shadow-xl py-4 transition-all duration-200 origin-top transform ${
                  servicesDropdownOpen ? 'scale-y-100 opacity-100 visible' : 'scale-y-95 opacity-0 invisible'
                }`}
              >
                <Link 
                  to="/services" 
            ...
            {navLinks.filter(l => l.path !== '/').map((link) => (
              <Link
                ...
                to={link.path}
                className={`text-[11px] xl:text-xs font-bold uppercase tracking-widest transition-colors duration-200 whitespace-nowrap ${
                  isActive(link.path) ? 'text-hbrz-gold border-b-2 border-hbrz-gold' : 'text-gray-500 hover:text-hbrz-blue'
                } py-2`}
              >
                {link.name}
              </Link>
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 py-6 max-h-[80vh] overflow-y-auto">
          <div className="px-4 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-4 py-3 rounded-md text-base font-bold uppercase tracking-wider ${
                  isActive(link.path) ? 'text-hbrz-gold bg-gray-50' : 'text-gray-600 hover:text-hbrz-blue'
                }`}
              >
                {link.name}
              </Link>
            ...
            <div className="border-b border-gray-50 pb-2">
              ...
              <div className="pl-6 space-y-1">
                <Link to="/services" className="block px-4 py-2 text-sm font-medium text-gray-500 uppercase tracking-wider">Overview</Link>
                {servicesSubLinks.map((sub) => (
                  <Link
                    key={sub.name}
                    to={sub.path}
